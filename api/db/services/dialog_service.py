...
def chat(dialog, messages, stream=True, **kwargs):
    ...
    langfuse_trace = None
    langfuse_connection_context = None
    embd_mdl = None
    chat_mdl = None
    rerank_mdl = None
    tts_mdl = None
    ...
    try:
        ...
        # 使用连接池获取Langfuse连接
        try:
            langfuse_connection_context = get_langfuse_connection(dialog.tenant_id)
            langfuse_tracer = langfuse_connection_context.__enter__()
            if langfuse_tracer:
        ...
        def decorate_answer(answer):
            ...
            # langfuse code
            langfuse_output = {"output": answer, "reference": refs, "prompt": re.sub(r"\n", "  \n", prompt)}
            try:
        ...
        # 清理Langfuse连接
        if langfuse_connection_context:
            try:
                langfuse_connection_context.__exit__(None, None, None)
            except Exception:
                pass
